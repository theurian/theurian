...
| **Artifact verification is not implemented** | Every release publishes `SHA256SUMS` and a CycloneDX SBOM, so the record a verifier would check against exists — and **nothing in Theurian checks it**. Setup never obtains Core: it looks for a `theurian` already on `PATH`, so it holds no downloaded artifact to hash and there is no point in the flow where such a check would run. `artifact-integrity` reports `not-applicable` rather than claiming a check it did not make — `theurian setup --dry-run` prints it, and installs nothing. Checking a download against `SHA256SUMS` is a manual step ([#39](https://github.com/theurian/theurian/issues/39), T-16). |
