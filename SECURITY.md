...
- **Sensitivity boundaries (enforced on the build, and now on the answer).** The
  ...
  *sensitivity* serving decision**, and that decision is now made elsewhere rather
  ...
  So the build-time partition this bullet describes is exactly what bounds that
  title; it is not a serving control layered on top. The scope key itself is real
  and tested over all 64 component combinations (#115).
...
- [Threat model](docs/security/threat-model.md) — trust boundaries and the enumerated threats; its threat summary table is the authoritative register
