...
| A repository contributor | Nothing. Their migration arrives by commit, so in the checkout that carries it, it is tracked and its working-tree bytes are the bytes at that commit — which is exactly what the predicate asks | **Zero, and that is the row that shows what the floor is.** Decision 1 asks *committed*, and this actor's whole route is committing; the control between them and approved knowledge is the human reviewing the pull request — ADR-0013 point 4, not this ADR |
