...
| R-3 | RAPTOR summaries leak facts not present in the source | Knowledge platform emits fiction | Milestone 6, not yet shipped: the design is extractive-first, with opt-in abstractive summaries that *would* carry `summary_model` provenance and be validated by an entailment check, every node keeping child references. The extractive half is built and unused: `infrastructure/raptor/extractive.py` selects sentences, so it cannot state a fact its children do not contain, and nothing calls it. `nodes.summary_model` exists at index schema v4 with no writer, and there is no entailment check anywhere in the tree. Interim residual: no summary is generated at all, and so none can emit fiction (#115). |
...
| R-7 | Index build blows up wall-clock and memory on a large monorepo | Setup appears hung | Incremental builds by default; builds are cancellable; progress is reported; the old index keeps serving. (The RAPTOR forest that *would* further limit blast radius is Milestone 6 and unbuilt — `infrastructure/raptor/` holds the default summariser and no builder or traversal — so it is design, not a shipped mitigation, #115.) |
...
| R-14 | Cross-tenant leakage through a shared RAPTOR node | The most severe correctness failure possible | Milestone 6, not yet shipped: a tree identity of project + tenant + sensitivity + ACL group + namespace + status *would* make mixing structurally impossible rather than policy-checked. `domain/raptor.py`'s `SummaryNode` refuses children whose scope disagrees with its own, which is that refusal at the value level, but nothing builds or traverses a node and nothing writes one into the `nodes` table index schema v4 adds to hold it — `infrastructure/raptor/` holds a default summariser that nothing calls — so the control is not live yet; the scope key that would partition the forest is real and asserted exhaustively (`test_scope_isolation.py`), but the forest that would use it is Milestone 6. Interim residual: no RAPTOR summary is generated, so there is nothing to mix (#115). |
