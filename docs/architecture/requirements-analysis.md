...
| R-14 | Cross-tenant leakage through a shared RAPTOR node | The most severe correctness failure possible | Milestone 6, not yet shipped: a tree identity of project + tenant + sensitivity + ACL group + namespace + status *would* make mixing structurally impossible rather than policy-checked. `infrastructure/raptor/` is a docstring-only package, so no summary node exists yet; the scope key that would partition the forest is real and asserted exhaustively (`test_scope_isolation.py`), but the forest that would use it is Milestone 6. Interim residual: no RAPTOR summary is generated, so there is nothing to mix (#115). |
