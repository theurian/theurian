...
| T-15 | A secret in a document becomes an approved, indexed revision | Information disclosure | High | SEC-11 — `theurian propose accept` scans every body it would land, and the migration document's own author-written fields with it: the title, description, labels, scope paths, `contentType` and the date fields, each operation's free text and chosen names, and every string of a source anchor (provider, sourceUri, filePath, repository, externalId, commitSha, blobSha) ([#336](https://github.com/theurian/theurian/issues/336)). `block` by default per `security.secretScan`, with a best-effort in-house detector; human review of the authored migration (ADR-0013) and supersede/retire with the withdrawal→purge trigger stand beside it. What it does not reach: the document's derived fields — the ULID- and hash-shaped identifiers, the fixed vocabularies and `contentFile`, each barred by a mechanism rather than by choice — the artifact level (YAML comments and the migration and body filenames, [#349](https://github.com/theurian/theurian/issues/349)), and a proposal's `evidence.json`, which `accept` never lands ([#330](https://github.com/theurian/theurian/issues/330)). Ingest-time and index-time scanning are separate controls and do not ship ([#198](https://github.com/theurian/theurian/issues/198)) |
