...
tree — a grep for `import theurian` and a `find` for `*.py` outside a `tests/`
path — and they are CI steps rather than tests, so they still fail when the
Python suite is broken. `test_plugin_boundary.py` asserts the same two
properties, but only for `plugins/claude-code/`.
