...
- **ADR-0003's "the port set is exactly these fourteen" is already broken, and
  ...
  **`McpClientConfig`, whose own module docstring calls itself a port
  ...
  > the register, with both non-members and their reasons. Two corrections to
  > the paragraph above, recorded rather than applied silently:
  >
  > - **The figures moved.** Re-measured on 2026-09-03 at `e2a950ef`:
  >   `ALL_PORTS` holds **17**, against **19** `Protocol` classes under
  >   `domain/ports/`. The 15-and-17 above is the `f702736` reading and stands
  >   as history. `ReviewFindingSource` and `ReviewFindingStore` are the two
  >   that joined, both under ADR-0029 — so the "adding a port requires an ADR"
  >   path was followed for these, unlike `IndexStore`.
  > - **"a read session `CanonicalStore` returns" was never true.** No
  >   `CanonicalStore` method returns a `CanonicalReadSession`; builders receive
  >   one from an injected `store_factory: Callable[[Path], CanonicalReadSession]`.
  >   The claim was wrong at `f702736` too, so this is an error rather than a
  >   drift. The reason it is outside the register is unchanged in substance —
  >   it opens no substitution point `CanonicalStore` does not already cover —
  >   but the mechanism given for it was wrong, and a wrong mechanism in a
  >   planning record is consumed as a settled premise.
  >
...
| 4 | ~~"The port set is exactly these fourteen"~~ **Discharged.** | At `f702736`: `ALL_PORTS` held **15** against **17 `Protocol` classes** under `domain/ports/`, the claim standing in ADR-0003 (twice), `docs/architecture/overview.md`'s diagram (`PORT["14 ports (Protocols)"]`) and the root CHANGELOG ("fourteen ports as `Protocol`s"). **Re-measured on 2026-09-03 at `e2a950ef`: `ALL_PORTS` holds 17 against 19 `Protocol` classes** — `ReviewFindingSource` and `ReviewFindingStore` joined under ADR-0029, which is the ADR-per-port path point 5 asks for. Closed by [#140](https://github.com/theurian/theurian/issues/140): ADR-0003 point 5 now carries a Milestone 7 amendment naming **`ALL_PORTS`** as the register, the count as a dated measurement, and both non-members with the reason each is outside. The diagram is re-keyed to the register and carries no literal number. The CHANGELOG line **stands as Milestone 0 history** with a pointer added — every count in that dated block (`Fifteen ADRs`, `275 tests`, `sixteen enumerated threats`) is a filing-time record, and correcting one to today's value would falsify the record. Two docstrings were corrected in the same pass: `CanonicalReadSession` no longer says "not a fifteenth port" (an ordinal that drifts with the count it is pinned to), and `McpClientConfig` no longer calls itself a port. **Left open:** whether `McpClientConfig` *joins* `ALL_PORTS` — its own ADR-requiring decision, still on #140. Until #140's tests stage lands the derivation that recomputes both figures from `ALL_PORTS` and an AST walk of `domain/ports/*.py`, the numbers here and in the ADR are prose and can drift again |
