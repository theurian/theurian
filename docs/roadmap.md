...
- **Security debt that gates widening the agent write path.** These are partly
  ...
  | SEC-11 (secret scanning) | the approval gate, over the author-written bytes of both artifacts an acceptance makes canonical: `theurian propose accept` scans every body it would land and the migration document's author-written field values with it — title, description, labels, scope paths, `contentType` and the date fields, each operation's free text and chosen names, and every string of a source anchor ([#336](https://github.com/theurian/theurian/issues/336)) — `block` by default per `security.secretScan` (ADR-0027 decision 3), with an in-house best-effort detector | the artifact level — YAML comments and the migration and body filenames ([#349](https://github.com/theurian/theurian/issues/349)); ingest-time and index-time scanning, and draft-time advisory scanning together with a proposal's `evidence.json`, which `accept` never moves into the canonical tree — separate controls at separate points ([#198](https://github.com/theurian/theurian/issues/198), [#329](https://github.com/theurian/theurian/issues/329), [#330](https://github.com/theurian/theurian/issues/330)) |
