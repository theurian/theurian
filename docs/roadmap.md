...
- **ADR-0003's "the port set is exactly these fourteen" is already broken, and
  ...
  **`McpClientConfig`, whose own module docstring calls itself a port
  ...
  > - **"a read session `CanonicalStore` returns" was never true, and the first
  >   replacement for it was also wrong.** No `CanonicalStore` method returns a
  >   `CanonicalReadSession`; that much was measured and holds. But this row
  >   then said builders receive one from a shared
  >   `store_factory: Callable[[Path], CanonicalReadSession]`, and the two
  >   consumers differ: `RetrievalService` takes that annotation
  >   (`application/retrieval_service.py`), while `IndexBuilder` takes
  >   `Callable[[Path], IndexBuildSession]` (`application/index_builder.py`).
  >   The original claim was wrong at `f702736` too, so it was an error rather
  >   than a drift — and replacing one wrong mechanism with another is why this
  >   bullet now names both sites instead of one shared spelling. The
  >   conclusion is unchanged throughout: what an operator substitutes is still
  >   a `CanonicalStore` adapter, so neither session opens a substitution point
  >   the register does not already cover.
  >
