...
- **Security debt that gates widening the agent write path.** These are partly
  ...
  | SEC-11 (secret scanning) | the approval gate: `theurian propose accept` scans every body it would land, `block` by default per `security.secretScan` (ADR-0027 decision 3), with an in-house best-effort detector | the revision's own metadata is unscanned — title and source anchors (provider, sourceUri, repository, commitSha, filePath) published verbatim on every result, description and labels committed but not published ([#336](https://github.com/theurian/theurian/issues/336)); ingest-time and index-time scanning, and draft-time advisory scanning — separate controls at separate points ([#198](https://github.com/theurian/theurian/issues/198)) |
