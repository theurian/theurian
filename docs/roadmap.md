...
- **Security debt that gates widening the agent write path.** These are partly
  ...
  | SEC-11 (secret scanning) | the approval gate, over both artifacts an acceptance makes canonical: `theurian propose accept` scans every body it would land and the migration document's author-written fields with it — title, description, labels, scope paths, each operation's free text and chosen names, and every string of a source anchor ([#336](https://github.com/theurian/theurian/issues/336)) — `block` by default per `security.secretScan` (ADR-0027 decision 3), with an in-house best-effort detector | ingest-time and index-time scanning, and draft-time advisory scanning together with a proposal's `evidence.json`, which `accept` never moves into the canonical tree — separate controls at separate points ([#198](https://github.com/theurian/theurian/issues/198), [#329](https://github.com/theurian/theurian/issues/329), [#330](https://github.com/theurian/theurian/issues/330)) |
...
| **Migration** | None. |
| **Security** | **The preconditions are already recorded in the threat model.** SEC-10's repository and URL allowlists are owed ([#129](https://github.com/theurian/theurian/issues/129)). SEC-11 (secret scanning) ships at the approval gate — `theurian propose accept` scans every body it would land **and the migration document's author-written fields with it** ([#336](https://github.com/theurian/theurian/issues/336)), `block` by default per `security.secretScan` (ADR-0027 decision 3) — so the title and the source anchors, which are published verbatim on every result, are covered at the gate; ingest-time and index-time scanning remain owed under [#198](https://github.com/theurian/theurian/issues/198) and [#329](https://github.com/theurian/theurian/issues/329), and a proposal's `evidence.json` under [#330](https://github.com/theurian/theurian/issues/330); the surfaces that describe what is and is not in force are regression-pinned by `test_config_key_call_sites.py` and `test_examples.py`. SEC-12 (JSON Schema validation of MCP input) becomes mandatory the moment a write-intent tool opens. **T-15's "nothing enforces the merge" residual is a Phase B precondition, not a background fact**: opening a protocol-level write path multiplies the callers who can put a file in `.theurian/migrations/`, and `migrate apply` does not ask whether it was committed. `evidence.json`'s `agentId` and `model` become the only record of origin in a multi-agent setting, so their being required is preserved. |
