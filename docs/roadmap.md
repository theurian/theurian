...
| **Exit criteria** | 0.1.0 live on PyPI · `pre-1.0` open = 0 · every appendix contradiction cleared · SECURITY.md and README pointing at the release authority — the `core-v*` tag list and PyPI — instead of asserting a version the next cut falsifies · **the T-16 decision recorded — implement install-time verification, or record it as a non-goal with the reasoning**. Without this the Architecture row asks for a decision that nothing checks was ever taken. |
...
- **ADR-0003's "the port set is exactly these fourteen" is already broken, and
  ...
  **`McpClientConfig`, whose own module docstring calls itself a port
  ...
  > which closed with the amendment and therefore holds nothing; the live
  > record is ADR-0003 point 5's amendment, in the `McpClientConfig` row of its
  > *Protocols outside the register* table. No issue owns it as of 2026-09-05
  > (appendix item 4 carries the search).
