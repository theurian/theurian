...
| **Exit criteria** | 0.1.0 live on PyPI · `pre-1.0` open = 0 · every appendix contradiction cleared · SECURITY.md and README pointing at the release authority — the `core-v*` tag list and PyPI — instead of asserting a version the next cut falsifies · **the T-16 decision recorded — taken on 2026-09-05: install-time artifact verification is a non-goal for 0.1.0**. The reasoning is in [the threat model's T-16 entry](security/threat-model.md), which is the record the Architecture row asks for; the unbuilt control keeps its own owner, [#80](https://github.com/theurian/theurian/issues/80). |
