...
- **ADR-0013 no longer describes the proposal-age report as shipped**
  ...
  that `doctor` warns past a threshold. Neither exists: `knowledge_status`
  publishes `itemsByStatus`, `itemCount`, `stateHash`, `appliedMigrations`,
  `schemaVersion` and a conditional `integrity`, the response schema names no
  proposal field, and nothing in the setup layer reads `.theurian/proposals/`
  at all. The bullet now reads as owed rather than shipped and names
  [#414](https://github.com/theurian/theurian/issues/414), which owns building
  both. Same class as the #195, #198 and #129 corrections — a mechanism named
  in the present tense in a durable record whose component does not exist —
  and it aligns the ADR with `docs/protocol/mcp-tools.md`, which already
  records that proposal ages are not in that response.
