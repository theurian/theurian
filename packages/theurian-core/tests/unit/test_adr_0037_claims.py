...
- **The no-new-disclosure bound** (decision 7). ``result_payload``'s exact
  published key sets, including the two ``sourceAnchor`` fields it does *not*
  publish -- which is why the ADR drops them rather than exporting them.

Pure: it reads ``domain/enums.py``'s constants, one JSON schema and one Markdown
...
# ---------------------------------------------------------------------------
# 4. The no-new-disclosure bound (decision 7).
# ---------------------------------------------------------------------------
...
def test_the_served_payload_publishes_exactly_the_keys_the_okf_bound_is_drawn_at() -> None:
    """Decision 7's universal claim is bounded by this key set and nothing else.

    ADR-0037 says the projection *publishes no metadata the serve path withholds*,
    and then refuses to defend that field by field: the bound **is**
    ``result_payload``'s published shape. Two of its consequences are recorded as
    decisions rather than as details -- ``theurian_trust_level`` and
    ...
    ``externalId`` are dropped *because* this payload does not carry them. The
    ADR calls that "what makes this a universal statement about the front matter
    rather than a claim with a carve-out in it".
    ...
    assert set(payload) == SERVED_TOP_LEVEL_KEYS, (
        f"`result_payload` publishes {sorted(payload)}. ADR-0037 decision 7 draws its "
        f"`no metadata the serve path withholds` bound at exactly this set: a NEW key "
...
def test_the_adr_still_states_the_bound_and_the_two_fields_it_drops() -> None:
    ...
    Two fragments because each carries a different half. The first is the bound
    itself -- the field list the universal claim is drawn at. The second is the
    consequence a reader of the dropped table needs: ``blobSha`` and ``externalId``
    are absent from the bundle *because* they are absent from the payload, and a
    document that drops that clause leaves the two rows looking like a preference
    that a later slice may reverse.
    """
    _assert_the_adr_states(
        ...
        because=(
            "This enumeration *is* decision 7's bound. Without it, `the projection "
            "publishes no metadata the serve path withholds` is a judgement rather "
            "than a measurement, which is the form the ADR explicitly refuses."
        ),
    )
    _assert_the_adr_states(
