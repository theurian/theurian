...
#: - ``infrastructure/git/committed_check.py`` runs ``git rev-parse --verify
#:   --quiet HEAD:<path>`` (the committed id) and ``git hash-object --stdin
#:   --path=<path>`` (the applied id) to answer whether a migration file is
#:   committed unmodified at ``HEAD`` -- the check ``migrate apply`` makes before it
#:   applies (ADR-0034, T-15). Like ``git log`` both read local object storage and
#:   contact no remote, so it is *not* a network client -- it is on this list only
#:   because it spawns a process. Each vector is the adapter's: the ``rev-parse``
#:   object argument begins with the literal ``HEAD:``, and the ``hash-object`` path
#:   is the ``--path=<value>`` form, so a migration filename cannot be read as an
#:   option or name a different revision, and neither call can be handed a URL or a
...
#:   --format= -z --root --end-of-options <sha>^{commit} -- <file_path>``,
...
#:   is verdict identical and 2.30 compatible. The z flag makes git emit NUL
#:   delimited raw path bytes rather than its human rendering, and a verdict is
#:   verified only when the exact stored path, encoded to UTF 8, is one of those
#:   byte entries after empty entries are dropped: no quoting, no line splitting,
#:   no whitespace stripping, no pathspec breadth. Reading the human rendering
#:   instead was the root cause of three findings, closed by this byte comparison
#:   (round 3); it also refuses a stored directory such as a bare dot, since a raw
#:   entry is one full path and never a directory (round 2 HIGH 2). The remaining
...
#:   first parent and merge diff flags let a conflict resolving merge be one. The binary is
...
def test_no_module_outside_the_daemon_health_probe_reaches_a_network_client() -> None:
    ...
    **What it cannot see.** It reads names -- though a name written as a string
    ...
    assert sites == sorted(NETWORK_CLIENT_SITES), (
        ...
        + "\n\nT-7 (SSRF) has no scheme allowlist and no private-network "
        "rejection in the raw-URL context; #429 owns both against the OpenAPI "
        "`$ref` fetcher. SEC-10's repository allowlist is no longer among them: "
        "it is read and enforced on the `gh` review-ingestion path (ADR-0030). "
        "What stands in for the two that are still owed is that no module "
        "outside the daemon's loopback health probe reaches a network *client* "
        "at all -- the review adapter is a spawn, not a client, which is why the "
        "sibling arm below is the one that sees it.\n\n"
        "If you added a client, docs/security/threat-model.md (T-7), "
        "docs/architecture/review-knowledge.md and "
        "plugins/claude-code/commands/ingest.md all describe a reach that has "
        "changed. Before listing the new site here, establish: the destination "
        "cannot be chosen by ingested content; the scheme and the network it may "
        "reach are checked; and a test goes red when either stops holding. Then "
        "correct those documents in the same change."
    )
...
def test_parsing_a_document_with_an_external_ref_opens_no_socket(
    ...
    assert attempts == [], (
        ...
        "rejection are owed against this `$ref` fetcher (#429) and enforce "
        "nothing on it today. "
...
def test_parsing_a_hostile_document_opens_no_socket(
    ...
    assert attempts == [], (
        ...
        "private-network rejection are owed against this `$ref` fetcher "
        "(#429) and enforce "
        "nothing on it today. Recording a reference is the shipped control; "
        "resolving one is the vulnerability."
    )
