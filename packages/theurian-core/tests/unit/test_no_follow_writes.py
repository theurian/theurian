...
_PUBLISHERS_OUTSIDE_THE_CLASS: Final[dict[str, str]] = {
    ...
    "application/project_service.py::ProjectRegistry._write": (
        ...
        "increase. **The cost this used to cite has since been paid** -- it said "
        "`index_commands.py::index_build` calls "
        ...
        "`--json` caller as a traceback. That call is now "
        "`_record_the_provenance`, which converts an `OSError` into a "
        "`provenanceWarning` beside the build report (#551 round one, security "
        "H-B). What remains is the *delivery* half alone: the temporary is not "
        "clone-deliverable, so a guard here would refuse nothing a repository can "
        "reach -- and `_UNGUARDED_LINK_REMEDY` is what the loop at that path "
        "publishes instead."
    ),
