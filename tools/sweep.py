...
#:
#: It also buys what it was built for: without it, most of the population
#: `tools/mutate.py`'s `_lend_git_objects` docstring states as a key and a
#: command (not a count -- three prior counts here each went stale) skip, so a
#: mutation whose only killer is one of them comes back SURVIVED from a run
#: that never executed the test holding it. For an unattended job that files
#: what it finds, that is a fabricated finding.
#:
